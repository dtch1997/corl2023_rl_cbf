...
class QNetwork(nn.Module):
    ...
    def get_barrier(self, x: torch.Tensor, barrier_threshold: float = None) -> torch.Tensor:
        ...
        barrier_threshold = torch.tensor(barrier_threshold, dtype=torch.float32, device=self.device)
    ...
    def predict_barrier(self,  x: torch.Tensor, barrier_threshold: float = None) -> np.ndarray:
        """ Predict barrier values """
        x = torch.tensor(x, dtype=torch.float32, device=self.device)
        barrier = self.get_barrier(x, barrier_threshold=barrier_threshold)
        return barrier.detach().cpu().numpy()
    
    def predict_next_barrier(self, x: torch.Tensor, u: torch.Tensor, barrier_threshold: float = None) -> np.ndarray:
        """ Predict safety of next state according to barrier threshold 
