...
def evaluate_and_record_states(
    ...
    states = [state]
    imgs = []
    frame_indices = [int(max_episode_length * fraction) for fraction in [0.1, 0.3, 0.5, 0.7, 0.9]]
...
if __name__ == "__main__":
    ...
    total_frames = 500
    frame_indices = [int(total_frames * fraction) for fraction in [0.1, 0.3, 0.5, 0.7, 0.9]]
    ...
    for i, frame in enumerate(frames):
        ...
        rect = patches.Rectangle((0,0),1,1, edgecolor='black', facecolor='none', transform=axs[i].transAxes, clip_on=False)
